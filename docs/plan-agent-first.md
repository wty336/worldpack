...
**三包终验（2026-09-24，factcheck 提示词精修后）**：全部通过——

| 包 | ooc | setting | confab | normal 误报 |
| --- | --- | --- | --- | --- |
| 江湖旧梦 | 100% | 100% | **100%** | 8% ✓ |
| 问道长生 | 100% | 100% | 92% | **6%** ✓ |
| 霓虹深处 | 100% | 95% | 93% | **0%** ✓ |

- **抽取器精修一轮**（"自述糗事不算 + 外部世界事件算"）后 xianxia 图层误报清零、
  normal 10.9% → 6%；
- **已知限制（如实记录）**：confab 的"场景同名"难例漏判——用例场景本身含断言地点
  （如"丹峰"），2 字接地规则放行（xianxia 2 条、urban 2~3 条），flash LLM judge
  同样 3/3 放行；v2 方向 = 按锚点类型加权接地（地点名权重降低），本期不做；
- 报告：`reports/judge_sensitivity_20260924-{224732,225525,230405}.json`。
