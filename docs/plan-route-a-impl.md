...
### 生产批进度（2026-09-13，清场后）

| 层 | 模块 | 目标 | 出库 | 丢弃 | complete | 备注 |
| --- | --- | --- | --- | --- | --- | --- |
| train | extract | 1000 | **998** | 2（标签不成立） | ✅ | 首批 200 先出库核验，再一条命令续到 1000（零重产） |
| train | judge | 800 合格 | **1093** | 375（含 912 定向跳过） | ✅ | 供卡 1060 + **定向补产 408**（confab 段 + setting 段）；家族配额达标 |
| train | compress | 300 | **290** | 10（3.3%） | ✅ | 长输入 97/290 = **33%** ≥ 决策 18 门槛 20% |
| train | **合计** | — | **2381** | 387（11.6%） | ✅ | **train 层完成（2026-09-14）**；配额：confab 41.8% / setting 26.2% / ooc 32.0% |
| dev | extract | 1000 | 200（第 1 段） | 0 | ⏳ | 第 1 段核验通过：0 丢弃、标签自检 0/32、孪生轴值（抗战谍战/日记体）产出正常 |
| dev | judge / compress | 800 合格 / 300 | — | — | — | 方案已算准（见下）；judge 基础段用**自然配比**、补产段定向 |
| eval | 三层 | — | — | — | — | 单独重跑 + 冻结打 tag（决策 14 的 VOID 流程） |
