...
| — 生产前加固：分片产出 + 断点续跑 | ✅ 完成 | `63a9d61`（+ 文档提交） | 无计划缺陷；评估出两个**能力缺口**（中断 = 全损 / 分模块发布覆盖 manifest）已补，见执行记录 |
...
- **人读出库清单** `scripts/scenario_factory/factory_review.py`（2026-09-13，首批 200 张后补）：
  把 JSONL 物证渲染成 Markdown（总览 → **风险项优先** → 丢弃明细 → 全量索引 → 标签总表
  ＋全文附录两份），因为"只有人能判"的东西不少（决策 16 的 confab 断言、标签是否被当成本身
  所述的那种东西成立、叙事像不像人写的），量一上来就没有可读入口了。首版把 40 条**负例**
  当风险项 → §2 的 64 条"风险"里 40 条是负例，真正的 1 条标签自检点名**彻底看不见**；
  负例是卡面的正常设计（正负例 4:1），已剔除出风险项并加守卫（`tests/test_factory_review.py`，10 条）。
- **extract 标签完整性测量**（2026-09-13，结论文档 `reports/extract-label-audit-20260913.md`）：
  原有检查**全是"声明 → 文本"方向**（anchors 在位 / 标签成立），反方向"文本 → 声明：有没有
  **多出**未标注的状态性事实"从未被覆盖 —— 而它决定 extract 的训练信号（多出未标注 → 教模型漏抽）。
  补测三次（开放提问 / 只让模型抽取、比对交代码 / 整张卡面当允许集）**三次都是噪声大于信号**
  （报的是氛围描写、已标注事实的换词复述、普通名词当专名）——**承认测不出来，也不报一个自己不信的比率**。
  诊断反而定死了结构性结论：卡面 `events` 是**刻意设计的闲谈**
  （`cards.py:310/321/325/327`：`noise="日常寒暄"`、`玩家与{n2}提起{n3}`，`n2/n3` 取自池子且
  **故意不是** facts 的实体；实测 sc-10000 两套实体完全不重叠）→ **回合里出现未被标注的专名是
  设计使然**（教的正是"满篇闲谈里只抽那几条状态事实"），不是缺陷，故不加门禁。
...
> **Task 9 之后各轮追加**：定 X 校准 4 → ⑨/⑩ 系列修复轮 → **断点续跑 17**（新文件
> `tests/test_factory_resume.py`）——**现基线 473 passed**。
...
### 决策 13 口径修定已落地：X = 一档容差 50pp（2026-09-13，经拍板）

**改动**：`rubric_x_calibrate.py` 里 X 的取值从 `round_up_to_5pp(p95(diffs))` 改为常量 `GRADE_TOLERANCE = 0.50`
（评委保真维三档折半后的一档）；旧口径降级为**诊断项** `x_p95`（报告仍打印，留痕对比）；
`calibrate()` 新增 `flagged` = 差超过一档的样本 id（**冲突清单**，直接对应决策 13 的"冲突进人读"）；
`format_md()` 的 ② 段写明新定义与修定理由。

**守卫同步**（3 条改动）：X = 0.50、`x_p95` 仍是旧口径、`flagged` 要含 diff > 一档的样本、报告含"一档容差"与"冲突"字样。

**重算结果**：**X = 50pp**；诊断 P50 = 0.0pp / P95 = 100.0pp / max = 100.0pp；
**冲突 2/24**：`sc-22025-0025`、`sc-22007-0007` —— 共同形态「**专名在位 ≠ 要点在位**」
（要点要求的承诺没写、而两个专名都在摘要里）⇒ **正是该进人读的样本，旧口径会把它们静默放过**。

### 三层带门禁重跑（2026-09-13，收尾）

| 层 | 出库 | 丢弃构成 | judge 类别（ooc/set/confab） | confab 幸存 |
| --- | --- | --- | --- | --- |
| train | **74** | 标签不成立 11 / 演绎丢弃 1 / 候选全杀 2 / 标签不一致 1 | 6/6/5 | 5/8 |
| dev | **81** | 标签不成立 6 / 候选全杀 3 | 9/8/7 | 7/10 |
| eval（冻结） | **77** | 标签不成立 12 / 候选全杀 1 | 2/5/11 | 11/12 |
| **合计** | **232 条**（门禁前 261） | — | judge 共 **59**（门禁前 87） | — |

id 三层唯一 ✓、9 个 manifest 摘要复核一致 ✓；eval 层**第三次**触发决策 14 例外（OPEN_LOG 第 3 条 VOID + 新 tag
`eval-route-a-20260913c`，旧 tag `…13b` 保留为作废留痕）。本轮成本 **¥2.410**（其中标签校验 254 次 ¥0.520）。

**验收证据（本项要证的事）**：对新三层 judge 批做**全量**标签复检（59 条，非抽样）——
判"问题类型不成立" **3/59 = 5%**（门禁前 **41%**），残余 3 条是**校验器复查抖动**（同型号两判不一致）。
⇒ **judge 批不再含门禁前那类问题样本** ✅

**但代价必须记下来（新增风险，抬高了"结构性修复"的优先级）**：门禁是"验不过就丢"，于是
**judge 批从 87 条缩到 59 条（−32%），且类别更不均衡**（eval 层 ooc 只剩 **2** 条，而配额要求 ooc/setting/confab
各 ≥20）✗。根因是那 ~7–13% "结构性救不回"的卡（包内 NPC 与名字池角色是**两套名字宇宙**，被声明的说话人
常常根本没开口 → 无论重演几次都判不出 OOC）。
⇒ **建议：把"结构性修复"提到生产批之前**（让被声明的说话人真的开口/让矛盾必然落在它身上），
否则 judge 批会一直又薄又不均。

### 发现⑨ 结构性修复已落地（2026-09-13）

**根因**：判定"问题类型是否成立"必须落在**被声明的说话人**身上，而原提示**没告诉演绎器说话人是谁**（更没给角色卡）
→ 实测 narration 常由别人说话、或说话人沉默 ⇒ 重演再多次也判不出 OOC，门禁只能丢卡。

**改法**（核心 = **让生成指令与 §7.5 验收判据对齐**）：
- `materialize.speaker_card_text()` 成为"说话人是谁、该怎么说话"的**单一真源**（演绎器指导生成 + 出库器随样本交付）；
- `verbalize._speaker_staging()` 取**展示名 + 角色卡全文**；`_judge_hint()` 补四条硬要求：
  ① 出现说话人的**直接引语**；② 矛盾**发生在他自己的话里**（不得旁人转述、不得模糊指代省专名）；
  ③ 角色卡只用于把握语气、**不得逐字照抄**（照抄会触发 `hook_gate`）；④ **anchors 必须原样在位**（防顾此失彼）；
- ooc 判据明写「说了与人设**相冲突的话**」而非「没展现其风格」——后者正是实测被误当 OOC 的来源。

**实测（dev judge-only 探针，各 30 张卡）**：卡级 `标签不成立` **6 → 1**；
出货样本判不成立 ①41% → ②5% → ③**0%** → ④12%（对照表：`reports/judge-label-stages-20260913.md`）。

**主动停手（纪律）**：④ 与 ③ 之间只差两句约束，差异**无法与 24 条样本抖动区分**——校验器自身噪声底 5~8%
（同一批样本**复查时判定会翻转**）。继续调提示词就是在噪声上拟合（与 Task 10 拒绝过的同类错误）⇒ 停。
要再往下走只能靠**人读**（`reports/judge-manual-review-20260913.md` 那类表），不能再靠单条 LLM 校验器。

**产量未提升（24/30）**：失败模式搬家到既有的 `演绎丢弃`（anchors 未在位，4 张）与 `confab 撞卡`
（≥4 字逐字重合，1 张）——两道门都在该在的位置，代价 ~5/30 张卡记在账上。

**待拍板**：要不要用修复后的流程**再刷一遍三层**（≈3 小时 / ¥5 / eval 第 4 次例外），
还是把这次修复**直接折进生产批**（生产批本来就要重造 1000/800/300，且需先定 P0 长输入档）。

### P0 长输入档：口径修定 + 分块续写（2026-09-13，经拍板）

**测量先行**（从真实存档量，不靠估）：生产端被压缩的历史段 = `longrun-xianxia_wendao` **15,557 字**、
`smoke-local14b-xianxia_wendao` **12,179 字**、其余 5 个 2,248~5,865 字
⇒ ① 长档不是空想；② **压缩质量只在这个长端才重要**。

**同时暴露**：档位原先对**所有模块**开放（600/3000/20000），而 extract 的输入是"一个回合"、
judge 的是"一段叙事"——生产端不可能出现 20K token 的它们 ⇒ 那批量卡在做**无意义的超长输入**，
真正需要的 compress 长档反而产不出来（实测 20K 档最长只产出 4.3K token）。

**改法**：
1. `cards.TIERS_BY_MODULE`：extract/judge/reflect 只留 `(600, 3000)`；compress 留 `(600, 3000, **12000**)`
   （长档目标按实测由 20000 重定）；`LONG_INPUT_TOKENS` 单一真源移入 `cards.py`。
2. `verbalize._verbalize_chunked()` **分块续写**：每块 ≤ `CHUNK_TOKENS=3000`；块间回喂上一块结尾
   要求"紧接、不重复、不总结"；拼完对**整段**做 anchors 校验；缺漏**只补最后一块**（整卡重演对长卡要 ×4~5 倍调用）。
3. `_chunked_user()`：**按块给长度**（"本段约 N 字"）——只给全篇目标时每块只写 1.3~2.6K 字。

**实测（compress 探针 8 张，两轮）**：

| 轮次 | 长卡实测长度 | 达标 `long_input` |
| --- | --- | --- |
| 改前（20K 档） | 400 ~ 4,127 字 | **0/4** |
| 分块续写（只给全篇长度） | 5,321 / 6,093 / 9,992 / 10,488 | 3/4 |
| **＋按块给长度** | **9,816 / 11,807 / 8,408 / 8,777** | **4/4** ✅ |

**顺带印证**：④ 那条"超长降级为偏好"的修复是长档能成立的前提——长输入的摘要普遍 800+ 字，
按老口径会被"超长"全杀 ✗。

**成本**：长卡 1 → 4 次演绎调用；生产批 compress 300 张（约 1/3 长档）≈ **+300 次调用 ≈ +¥1.8**，
仍远低于 §10.2 的 ¥62 估算。

**守卫 +2**：档位按模块（extract/judge 无长档）、长卡分块且块间带续写指令；
**夹具改造**：`_chunked()` **按子句边界切**（否则 `"\n".join` 会把专名劈成两半 → 假红，实测踩过）。

### 生产前规模预演（2026-09-13，105 张卡，生产配比）

配置：`--layer train --extract 50 --judge 40 --compress 15 --sampling long --out <临时目录>`
（**全新输出目录**，不碰正式数据）。

| 指标 | 实测 | 判读 |
| --- | --- | --- |
| ① 丢弃率 | **13.3%**（batch 级门 30%） | ✓ 通过；**分模块**：extract **0%** ✓ / judge **30.0%** ✗ 卡门限 / compress 13.3% |
| ② 长输入档 | **6/6 达标**（7,480 ~ 12,124 字 vs 目标 12,000） | ✓ 分块续写在规模上成立 |
| ③ judge 标签复检（**全量** 27 条） | **0 不成立**（ooc 11 / confab 10 / setting 6） | ✓ 生成时门禁有效 |
| ④ 成本 / 耗时 | **¥1.884 / 430 次调用 / 25.1 分钟** | 外推生产批 **≈¥38 / 8.4 小时**（§10.2 估 ¥62；¥90 停批门未触 ✓） |

**预演直接抓到三件事**（这就是先跑 1 小时的价值）：

1. **整批丢失级 bug**：侧证据文件写在 `write_layer` **之前**，而全新 `--out` 时层目录还不存在
   → `FileNotFoundError` 把 **26.8 分钟 / ¥1.2 的成果全部丢掉**。此前每次都跑 `data/route-a/{layer}/`、
   目录早已存在，故一直没露头 ⇒ 抽 `write_side_file()` 自动建父目录 + 守卫。**若直接起生产批，第一层就会炸 ✗**
2. **事后抽检改为只报告不剔除**：它按**单条 LLM 判定**丢样本 = 双重惩罚 + 误杀——预演里它点名 3 条，
   独立**全量**复检判 **0/27 不成立**（其中 2 条是校验器对*同一样本*自我矛盾）⇒ 执行者归**生成时门禁**
   （它还能重演把样本救回来）；顺带把 judge 丢弃率 30% → 25%。
3. **丢弃明细落盘**：`dropped-*.json` 增 `detail` 字段——聚合键把 `confab 撞卡: 甲/乙` 截成 `confab 撞卡`，
   下一轮想修"撞的是哪些词"就无从下手。

**遗留（不阻塞生产，但要盯）**：judge 单模块丢弃率 **30%**（正好卡门限），最大单项是 `confab 撞卡` **12.5%**
——这是 ⑨「说话人舞台」的副作用（角色开口后台词更易与角色卡**逐字**重合）。若生产批 judge 丢弃率持续 >30%，
优先重定 `FACTORY_HOOK_N`（4 字 → 5 字，需按新语料重测）或让演绎器更明确地"换词不复述"。

### 发现⑩：元叙述/标签泄漏（人读初审抓到）——P1/P2/P3 已修并验证

**动因**：你先让 LLM 审了一遍 confab 清单（`reports/confab-manual-review-20260913-llm.md`，
19✅/3⚠️/2❌ + 五条系统性发现）。我逐条复核 + 量化到全批，结论是**初审的两个 ❌ 属实且严重**：

> 那两条**根本不是叙事**，是在念卡面 JSON：「`category 是 confab`」「`expect 栏标的是：问题类型，虚构事实`」
> 「`target_fact 指向 facts 数组的第 0 项`」「`权重 9`」「`scale 上 nodes 记 5，npcs 记 4`」
> ⇒ **标签直接泄漏进训练输入**。

**为什么四道门全漏**（这是本轮最值得记的一条）：程序校验问的都是"专名在不在 / 原词在不在 / 与角色卡撞不撞 /
要点成不成立"，**没有一道问"这到底是不是一段叙事"** ✗ —— 而这个维度正是**决策 16 的人读通路**抓到的 ✓
（该机制存在意义的实证）。

**量化（全批，不只 confab）**：硬元词命中（收窄口径后）**4/232 = 1.7%**，三个模块都有；
温和元词 42 条；货币穿帮 **12/59 = 20%**（judge 批，`urban_neon` 里写"五十两"）。

**改法**：① `META_HARD` 硬杀 + `_violations()` 统一校验（元叙述与缺 anchor 同权）；
② 系统提示第⑤条（"故事文本，不是任务说明/参数报告/卡面播报"）；③ `技术术语` 语体细则（只在叙事内用）；
④ `META_SOFT` 温和词只记不杀（误杀合法技术叙事要付吞吐代价）+ 落样本字段；
⑤ 金额/节令按题材（9 题材全覆盖）+ **judge 卡最终题材在造事实前定格**（原先重挑在造事实之后 → 事实与 pack 脱节）；
⑥ 近误人名上报（只报不杀）；⑦ 顺带：丢弃原因带明细（缺哪些 anchor / 元叙述命中哪些词）。

**验证（预演②，105 张卡，全新输出目录）**：

| 指标 | 预演①（修复前） | 预演②（修复后） |
| --- | --- | --- |
| 元叙述硬命中 | 1.7%（全批）/ 10%（judge） | **0/91 = 0%** ✅ |
| 货币穿帮 | 12/59 = 20% | **0/26 = 0%** ✅ |
| 长输入档达标 | 6/6 | **8/8** ✅ |
| 出库 / 丢弃 | 90 / 14（13.3%） | 91 / 14（13.3%） |
| 温和元词 / 近误人名 | — | 6 条（只记）/ 0 条 |

守卫 +5（449 → 454）。

**遗留（本轮**未**回归，已精确定位到明细）**：judge 丢弃率 **35%**，三项成因——
`confab 撞卡` 5（**照抄角色卡原词**：`冷静克制`/`没心没肺`/`医者底线`/`澜生生物`，是 ⑨ 的副作用）；
`演绎丢弃` 4（**只漏一个 anchor 名**：`铜罗盘`/`银铃`/`黄铜齿轮`/`青鳞`——可用"补漏块"式定点补救，像长卡那样）；
`标签不成立` 5（**全是 ooc**：模型把"违背人设"演成了"符合人设"）。extract 与 compress 都是 **0% 丢弃** ✓。

### ⑩ 遗留治理（B 轮，2026-09-13）：judge 丢弃率 35% → 22.5%

三处对症下药（都有明细证据）：

1. **ooc 指令给可执行写法**：原先只说"让台词明显违背角色卡" → 模型演成"沉默寡言/风格平淡" ✗。
   现给**三条具体写法**：语气反着来（卡面"冷静克制"→急躁咋呼爆粗）/ 踩底线（明写"不谈入门缘由"却主动大谈）/
   破禁忌（提及现代事物、说破自己是角色或 AI），并明说"**不是**没演出来"。
2. **舞台块加重**："**逐字照抄卡里任何 4 字以上的短语都会让这张卡作废**（出厂有程序检查）——必须换你自己的说法"
   （预演② 的撞词就是角色卡原文：`冷静克制`/`没心没肺`/`医者底线`/`澜生生物`）。
3. **定点补漏**（`_repair_missing`）：只缺 anchors 时**追加一次"把这些内容自然补进去"**的调用，
   而不是整卡重演（重演常常再漏同一个 ✗；实测 4 张卡都是只漏一个名）；**元叙述不补**（那是整段不是叙事，补不回来）。

**预演③（105 张卡，全新输出目录）三版对照**：

| 指标 | 预演①（⑩ 前） | 预演②（⑩ 后） | **预演③（B 轮后）** |
| --- | --- | --- | --- |
| judge 丢弃率 | 30.0% | 35.0% | **22.5%** ✅ |
| 出库 / 丢弃 | 90 / 14（13.3%） | 91 / 14（13.3%） | **95 / 10（9.5%）** |
| `confab 撞卡` | 5 | 5 | **2** |
| `演绎丢弃` | 2 | 4 | **3** |
| `标签不成立` | 3 | 5 | **4**（ooc 由 5 降到 2 条） |
| 元叙述硬命中 / 货币穿帮 | 1.7% / 20% | 0 / 0 | **0 / 0** ✅ |
| 长输入达标 | 6/6 | 8/8 | **8/8** ✅ |
| judge 类别（ooc/set/confab） | — | — | **12/7/12**（配额均衡 ✅） |
| extract / compress 丢弃 | 0% / 13.3% | 0% / 0% | **0% / 6.7%** |

**成本**：预演③ 470 次调用 **¥2.229 / 28.4 分钟**（105 卡）→ 外推生产批 2100 卡
**≈¥45 / 9.5 小时**；按 judge 22.5% 丢弃率，要出 800 条合格样本需**供卡 ~1032 张**（原计划 800）。
守卫 +2（定点补漏必被调用 / 元叙述不走补漏）；计数 454 → **456**。

**残余（已到收益递减，建议不再单独治理）**：judge 仍丢 22.5%，主要是 2 条 ooc
（模型仍会写"风格相符"）+ 各 1 条 confab/setting 的语义判读差异 —— 后者与校验器噪声底同级 ✗。

---

### 生产前加固：分片产出 + 断点续跑（2026-09-13，起跑前评估）

**缺口**（在 `assemble.py:main` 里**逐行核实**出来的，不是推测）：

1. **中断 = 全损**：样本只活在内存里，`write_layer` 是**最后一步**一次性落盘 → 十来小时的批
   （≈¥45）跑到第 9 小时断了（Ctrl-C / 网络超时 / 机器重启），**已花的调用费不可回收**。
2. **分模块跑会覆盖 manifest**：`write_layer` 的 `modules` 只记**本次**模块 → 先 `--extract` 后
   `--judge`，manifest 里就只剩 judge，`extract.jsonl` 还在盘上却**无人认领**
   （下游按 manifest 校验会**静默漏掉一个模块**）。同一模块再跑一次还会从 `i=0` 重做（重复付费）。

**做法**：新增 `scripts/scenario_factory/worklog.py`（产线工作日志）——每处理完一张卡就**立刻**
追加一行到 `{out}/{layer}/.work/{module}.jsonl` 并 `flush`。键是 `i`：`card_seed` 决定卡，而
`generate_card` 对 `(seed, seq, module)` 是**确定性**的（函数 docstring 就是这条契约），
故**续跑 = 跳过日志里已有的 `i`**。日志 append-only + **后写覆盖先写**，补判（质检/重造）只需再追加一行。
**发布改为幂等**：内容全部来自日志，分模块/分片发布得到同一份数据集 + 累积 manifest。

四个配套决定（每一条都有守卫钉住）：

| 决定 | 为什么 |
| --- | --- |
| **版本守卫**：日志头记 `factory_version`（提示词 + `cards.py`/`verbalize.py` **源码摘要** + 关键旋钮）+ 模型三元组 + 采样档；不一致**拒绝续跑** | 续跑最大的风险不是丢数据，而是**两代产线的样本混进同一份数据集**（改完 `VERBALIZE_SYSTEM` 接着跑：旧风格 400 + 新风格 600，出库时**看不出来**）。要重做只能 `--fresh` |
| **质检判定写回日志**（只判没判过的） | 旧版每次发布都对整层重抽 20%：分片发布累计多花 ≈¥11；更糟的是同一条样本**这次过、下次不过** → 出库内容随发布次数抖动 |
| **目标粘性**（`{layer}/.work/targets.json` 取历史最大值）+ manifest 增 `progress`/`complete` | 否则 `--extract 300`（将来要做 1000）发布出来的 300 条会被标成 `complete`，而它与成品**长得一模一样**。粘性目标**只进账、不驱动产出**（说好只跑一部分，就不能顺带把别的模块也跑了） |
| **指纹按层分文件**（`seen/{layer}.json`，纯函数于本层日志） | 旧版全局 `seen_fingerprints.json` 里删不掉某一层的指纹 → `--fresh` 重做时，重造文本与旧版**前 64 字往往一样**（同一张卡 + 同一演绎提示词）→ 被自己的旧指纹**全灭**（写测试时抓到的真陷阱）。旧全局文件不再读取 |

**真实演练**（真 API，非模拟；目录用完即删）：

| 步骤 | 结果 |
| --- | --- |
| `--extract 8` 跑到第 6 张时 **SIGKILL**（比 Ctrl-C 更狠，无收尾机会） | 日志已落盘 **5** 张、**零出库**（中断的批不留半成品数据集） |
| `--status`（零成本） | `extract 已产 5（保留 5 / 丢弃 0 / 质检剔除 0）待产 3` |
| 重跑同一命令 | 「**已完成 5/8，本次新增 3**」→ 出库 8 条；强杀前那 5 条**逐字未变**（sha 对照，未重产） |
| 再跑一次（幂等核对） | **usage 新增 0 行**、数据集**字节未变** |
| 随后 `--judge 2`（分模块发布） | manifest 同时认 **extract 8 + judge 2**（旧版此处只剩 judge） |

**用法**（生产批）：

```bash
# 分模块跑（想跑多少写多少）；中断了就把**同一条命令**再跑一遍
python -m scripts.scenario_factory.assemble --layer train --extract 300 --out data/route-a
python -m scripts.scenario_factory.assemble --layer train --status      # 零成本看进度
# 换代重做（已产样本作废，账要认）：加 --fresh；只攒不发布：--no-publish
```

守卫 +17（新文件 `tests/test_factory_resume.py`）：中断落盘 / 只补新增 / 幂等发布 /
分模块不丢模块 / 质检剔除**可重造** vs 门禁丢弃**不重造** / 换代拒绝 / 坏行不算已完成 /
重造时排除自己的旧指纹。计数 456 → **473**。

---

### 生产批进度（2026-09-13 清场后起跑；**09-14 三层齐**，09-18 补 ① 负例）

| 层 | 模块 | 目标 | 出库 | 丢弃 | complete | 备注 |
| --- | --- | --- | --- | --- | --- | --- |
| train | extract | 1000 | **998** | 2（标签不成立） | ✅ | 首批 200 先出库核验，再一条命令续到 1000（零重产） |
| train | judge（缺陷） | 800 合格 | **1093** | 375（含 912 定向跳过） | ✅ | 供卡 1060 + **定向补产 408**（决策 35）；家族 41.8/26.2/32.0 ✓ |
| train | judge_normal（负例） | 每题材 ≥40 | **221** | 39（**15.0%**） | ✅ | 仙侠 84 / 古代武侠 56 / 现代都市 81（决策 37） |
| train | compress | 300 | **290** | 10（3.3%） | ✅ | 长输入 97/290 = **33%** ≥ 决策 18 门槛 20% |
| train | **合计** | — | **2602** | 426（14.1%） | ✅ | **train 完成**；judge 家族合计 1314（缺陷 1093 + 负例 221） |
| dev | extract | 1000 | **994** | 6 | ✅ | 孪生轴值：抗战谍战 / 日记体（决策 19） |
| dev | judge（缺陷） | 800 合格 | **1177** | 431（含定向跳过） | ✅ | 家族 41.7/25.8/32.5 ✓（setting 余量仅 0.8pp，见决策 35 附注） |
| dev | judge_normal（负例） | 每题材 ≥40 | **210** | 50（19.2%） | ✅ | 仙侠 72 / 古代武侠 69 / 现代都市 69（决策 37） |
| dev | compress | 300 | **282** | 18 | ✅ | 长输入档达标 |
| dev | **合计** | — | **2663** | 470（15.0%） | ✅ | **dev 完成**；judge 家族合计 1387（1177 + 210） |
| eval | extract / judge / compress | 1000 / 1000 / 300 | **992 / 1046 / 283** | 368（13.7%） | ✅ **frozen** | 决策 14：单独跑整层 → 冻结打 tag `eval-route-a-20260918`；**不加负例**（尺子不补，理由见决策 37） |
| eval | **合计** | — | **2321** | — | ✅ | 三层总 **7586 条**（2602 + 2663 + 2321），id 全唯一、sha256 全对账 |

> `dropped-*.json` 的完整归因（三层合计 1264 张）：`演绎丢弃` 414 / `confab 撞卡` 383 /
> `标签不成立` 336 / `正常样本丢弃` 89 / `候选全杀` 42。

**train 层完成核对（2026-09-14）**：

- 三模块 **id 全唯一、manifest sha256 与文件实算全一致、`complete=True`**（progress 三模块 pending 全 0）。
- QA：货币穿帮 extract 1 / judge 0 / compress 0；近误人名全 0；温和元词 89 / 26 / 32（只记不杀）。
- 标签自检累计抽检 **381 条 → 不一致 15 = 3.9%**（低于校验器 5~8% 噪声底）。
- 丢弃 269 归因：`标签不成立` 77 + `演绎丢弃` 100 + `confab 撞卡` 84 + `候选全杀` 8。
- **成本实测 ¥28.44**（7211 次调用 / 2360 张卡 ≈ ¥0.0121/张）——**比 §10.2 估算的 ¥45 低 37%**；
  单价 extract ¥0.0066 / judge ¥0.0095 / compress ¥0.0227，吞吐 7.7 / 10.7 / **25.3** 秒每张。
- 三份人读清单：`extract-review-train-20260913.md`(998)、`judge-review-train-20260914.md`(803)、
  `compress-review-train-20260914.md`(290)；**confab 人读清单 238 条**（决策 16 待人工复核）。
- 残留观察（都不是缺陷，留给下一代数据集）：① judge 长度未兑现 200/1093（18%，同发现⑤ 一类，
  judge 无长度门槛）；② compress 超提示词目标 156/290（留痕，非杀）；③ **judge 材料重复 34%**
  （603 种/842 条，最多一份材料被 13 条共用）—— 挖到底后判定**良性**：45 种重复材料里 44 种是
  "同材料考同一类问题但 `detail`（矛盾依据）不同"，严格冗余只有 2 条；真正的含义是
  **材料多样性低于样本数**（材料由 `(pack, 第 N 天, 在场 NPC, facts)` 决定，池子有限）。
  ⇒ **待办**：dev/eval 产出时**核验跨层材料重叠**（eval 是尺子，材料撞了就是污染；现有跨层去重只管叙事前缀 64 字，不管材料）。

### 决策 35：judge 家族配额缺口与「定向补产」（2026-09-14，经拍板）

**发现**：`plan-phase1-data.md` §4.2.2 对 judge 训练集有**家族配额** ——
**confab ≥40% / setting ≥25% / ooc ≥20%**（理由：*"judge 训练量的分配必须向该家族倾斜，
否则平均分会被 ooc/setting 盖住"*，而 confab 正是学生模型最弱的一课，同难口径实测漏 ~70%）。
而 train judge 实测 **confab 仅 29.6%**，且**没有任何代码在检查它** —— `quota_gaps()` 只查题材占比与
compress 长输入档（§4.4 轴矩阵配额是**已记录的遗留**，这是它第一次产生实际后果）。
**教训：规格里写了配额，就必须有代码在检查它。**

**根因**：卡生成器的 judge 类别配比（confab 28.5% / setting 34.5% / ooc 37%）定在配额拍板
（2026-09-12）**之前**，从未跟改；再叠加各类丢弃率差异（setting 存活 64%、ooc 92%）把 ooc 抬得更高。

**为什么不改卡生成器**（原方案 C 的修正版）：改 `cards.py` → `factory_version` 变 →
**全层已产数据作废**（¥28 / 7.4h），还要连带做"模块级版本守卫 + 迁移"。而配额是**交付组成**的要求，
用**定向选产**满足它更省更安全：类别是 `(seed, seq, module)` 的确定性函数 ⇒ **零成本**就知道
哪个索引是哪一类，于是**只产需要的那几类**，其余索引记为 `skipped`。

**机制**（`--only-category`，judge 专有）：卡生成器一个字没改、`factory_version` 不变、
已产数据全部继续有效。`skipped` 是**独立状态**：算已完成（否则 `complete` 永远 False），
但**绝不算丢弃**（丢弃率是质量指标、还要过 30% 质量门，混记会同时污染两个口径）。
同时把家族配额与 §4.5「每题材每类 ≥5」**补进 `quota_gaps()`**（此前无人查）。守卫 +7，计数 491 → 498。

**执行**：confab 段 294 张（跳过 689）+ setting 段 88 张（跳过 188）+ 余量段 26 张（跳过 35）
= **定向补产 408 张 ≈ ¥3.9**，其余 912 个索引标记 skipped。
**结果**：judge **1093 条**，家族份额 **confab 41.8% ✓ / setting 26.2% ✓ / ooc 32.0% ✓**，
每题材每类最小格 **76**（要求 ≥5）。train 层合计 **2381 条**、`complete=True`、`quota_gaps` 零缺口。

> **setting 余量 0.0pp 的惊险一幕**：confab 段实测存活 74.5%（高于估算 69.2%）→ confab 多出 16 条
> → 把 setting 份额挤到**恰好 25.0%**；而**发布阶段的质检剔除会减样本**，剔掉 1 条就掉到 24.9% ✗
> ⇒ 补了余量段。**配额必须发布前实测、不能靠推算**；且**门的余量要覆盖"发布阶段的剔除"**。

### 决策 36：confab 材料形状泄露类别 —— 设计级缺陷与零成本修复（2026-09-14）

**发现路径**：从"dev judge 与 train 的材料重叠 43.8%"这条指标追下去。**我先误判为"材料空间整体太小"，
再靠一次矛盾测量把它纠正**：卡空间里"同材料 ⇒ 同类别"只有 18%、甚至 0%（三重交叉表全部均匀三等分），
而**交付集里"同材料 ⇒ 同类别"却是 98%** —— 矛盾逼出了真相：重叠的**只是 confab 那一块**。

**根因**：confab 卡的 facts **全部**标 `in_material=false`（`cards.py:340` + 其校验
"confab 卡的 facts 全部须 in_material: false"强制）→ 材料的「关键事实」段**整体消失**。
设计意图（编造的事实必须在材料里不存在）是对的，但用的是**最钝的实现**。后果三条（均已实测）：

| 后果 | 实测 |
| --- | --- |
| **① 材料形状 100% 泄露类别** | 规则「材料无事实段 ⇒ 虚构事实」train **457/457**、dev **491/491**，**零误报** ⇒ 判官**不读叙事**就能判出这一族；而**生产里材料永远有事实段** ⇒ 捷径在生产里失效，而 confab 正是学生最弱、配额 ≥40% 专为它设的那一课 |
| ② 与生产分布不一致 | 模型只见过"空材料版"的 confab |
| ③ 最该补的一族上下文最单调 | confab 材料空间塌成 **42 种**（setting/ooc 分别 631/675 种） |

**修法（①）**：把**非目标事实**重新物化进材料（**被断言的那条继续缺席**），**叙事与标签一字不动**
—— 叙事本来就带着非目标事实的 anchor（anchor 校验一直要求它们在位，实测 0 缺失），
只有材料渲染把它们漏了 ⇒ 两边**本来就一致**，补回去不引入新通路。**零 LLM 调用**
（材料是卡面的确定性重算），写盘用**日志 append 更新条目**（last-wins：可审计、不改历史、中断安全）。
脚本 `scripts/scenario_factory/fix_confab_material.py`（默认只预览，`--apply` 才落盘）。

**校验口径同步收窄**（`materialize._check_text` 校验③）：由"任一 `in_material=false` 的事实不得入材料"
改为"**被断言的那条**（`corruptions[0].target_fact`）不得入材料，其余自由"。**向后兼容**：
旧卡（不显式声明 `material.facts`）照旧通过 —— 全量 500 条守卫绿。

**验收（train 半，2026-09-14）**：
- 1093 条 judge 材料**全部含事实段** ⇒ 规则失效、捷径消失 ✓；
- **confab 材料空间 42 → 387 种**（总 673 → 1018 种）✓ 上下文多样性翻 9 倍；
- 不变量零偏差（`expect`/`detail` 与卡面一致、材料/叙事非空、**被断言 anchor 未泄漏**）✓；
- `git status` 只动 `judge.jsonl` / `manifest.json` / `confab 人读清单` —— **extract.jsonl 与 compress.jsonl 一字未变** ✓；
- dev 半作为天然对照组仍显示 491/491 = 100%，待 compress 跑完后同法处理。

**顺带记下的守卫缺口（下一代补）**：`materialize.py` **不在 `factory_version` 的指纹里**
（它决定样本的 `material` 字段）→ 改渲染逻辑不会触发保护；而**现在**把它加进去会让两层已产数据
全部失效 ⇒ 记录在案、留待下一代与"行为指纹版守卫"一起做。

**守卫 +5**（`tests/test_fix_confab_material.py`）+3（`tests/test_scenario_factory.py` 的 confab 材料口径）：
只改该改的（非 confab 一字不动、叙事/标签/质检判定保留）/ 预览不写盘 / 被断言 anchor 绝不入材料 /
非目标 anchor 必须补入 / 幂等（已补过的不再改）/ 硬要求不许被新口径放走。计数 498 → **500**。

**judge 首批 200 的丢弃归因**（逐条机械归因，不靠印象；`{layer}/.work/judge.jsonl` 的 reason 只报裸词，
"缺 anchor"与"原值出现"两种都可能，判据是**该词是否属于被腐蚀那条事实的 anchor**）：
| 类别 | 卡面 | 丢弃 | 存活率 | 原因分布 |
| --- | --- | --- | --- | --- |
| setting | 69 | 26 | **62.3%** | **原值出现（反向校验）22** + 标签不成立 4 |
| confab | 57 | 10 | 82.5% | confab 撞卡 8 + 标签不成立 2 |
| ooc | 74 | 6 | **91.9%** | 标签不成立 6 |
| 合计 | 200 | 42 | 79.0% | 演绎丢弃类 22（**全是原值出现**）+ 撞卡 8 + 标签不成立 12 |

- **setting 是丢弃重灾区**，且**全部**是反向校验：模型写出了卡面明确要求替换掉的旧值
  （`五十两`/`铜罗盘`/`青鳞`…）。卡面设计是自洽的（`_missing_anchors` 用 `i != hit_idx`
  跳过了被腐蚀的事实，不存在"必须出现又不得出现"的矛盾），是**提示词遵从**问题。
  代价纯在**供给**（通过的样本标签都是对的），且已计入供卡余量（1032 → ≈815 ≥ 800）。
- 预演③ 的 22.5% 在真批上得到验证（**21.0%**）；confab 撞卡率 14%（预演 5.7%，n 小）。
- 3 条标签自检点名（虚构事实 ×2 / OOC ×1）**同样本复检两次仍判"不成立"** ⇒ **不是随机噪声**，
  而是生成器与校验器对"什么算**直接断言**、什么算**与底线冲突**"的**口径分歧**（生成时同一校验器
  判它成立、自检时判不成立）；判定权在人读清单 §2。抽检 3/32 = 9.4%，按此推算满批 815 条里
  约 78 条带"可争议类别"——**报告-only 不剔除**（那正是决策 16 的人读面）。
- **不改**：修它要动 `verbalize.py` → `factory_version` 变 → 已产（extract 998 + judge 158，
  ¥8.5 / 4h）全部作废，换来的是每批多留 ~11% 的卡 ≈ ¥1 ⇒ 收益远小于代价；留给下一代数据集。

**实测成本/速度**（回填 §10.2 估算）：extract 每张 **¥0.0066 / 7.7 秒** → 1000 张 ≈ **¥6.6 / 2.15 小时**。
早先「≈¥45 / 9.5 小时」是按 judge/compress 混算的平均值外推的，extract 单项便宜约 3 倍；
judge/compress 单价待其首批实测回填。

**extract 首批核验结论**（详见 `reports/extract-review-train-20260913.md` 与清单 §2）：

- 硬指标全绿：0 元叙述硬命中、**货币穿帮 1/998**（时代币值的叙事内引用，真阳性但无害）、
  近误人名 0、id 唯一、sha256 对账一致。
- 标签自检累计抽检 161 条 → 不一致 4 条 = **2.5%**，低于校验器 5~8% 噪声底。
- 丢弃 2 张**同因**：`标签不成立：玩家的装备名为「密码本」`。逐条查实：携带该标签的样本
  70/998 = 7.0%，其中出问题 3 条 → **全层影响 0.3%**。修它要改 `cards.py` →
  `factory_version` 变 → **已产 1000 张（¥8 / 2.15h）全部作废**，且 train/dev 必须同一代卡空间
  ⇒ **决定不改**（此前只有"1/32 低于噪声底"的弱证据，现在是硬账）。
- 长度：实测不足卡面目标一半 39/998 = 3.9%（extract 无长度门槛，`long_input` 只对 compress
  的 ≥8000 档有意义）⇒ 只留痕不处理。

### 决策 37：judge 正常样本（负例）—— 0 条「通过」样本与家族配额的分母（2026-09-18）

**动因**：judge 卡**必带**一处腐蚀 ⇒ 三层 7155 条里 **0 条 `expect=通过`**。而 plan-phase1-data
§4.2.2 要求正常样本"每题材 ≥40"，门禁要求**误报率 ≤10%**（flash 基线 0%）：
只喂缺陷样本，判官会退化成"总能挑出毛病"，**第一个坏掉的就是误报率**。

**路径选择**：

| 路径 | 做法 | 代价 | 结论 |
| --- | --- | --- | --- |
| **A（采纳）** | 新增**附加模块** `judge_normal`（独立种子段 `seed_base+7000`、独立日志与配额、独立模块文件） | **¥5.07 / 约 1 小时** | ✅ 已产数据一字未动 |
| B | 改卡生成器加 normal 类别 | `cards.py` 变 → `factory_version` 变 → **三层期刊全废 → 重产 ≈¥96 / 45 小时**；且 **eval 已冻结（决策 14）⇒ 连重产的前提都不成立** | ❌ |

**做法**：`normal_card()` 由 judge 卡**去腐蚀**而来（同 pack / 同天 / 同 NPC / 同事实
⇒ 材料生成过程与缺陷样本**同源**）→ 同一条演绎提示词 → 三重门：① 叙事不得含该卡任何腐蚀
② anchors 须在位 ③ 独立 LLM 判"无问题"。**未判定不算通过**（"空 = 未知 ≠ 通过"：判不出就丢）。

**结果**：train 260 卡 → **221 条**（丢 39 = 15.0%）；dev 260 卡 → **210 条**（丢 50 = 19.2%）；
**每题材全部 ≥40**（train 84/56/81、dev 72/69/69）。三层合计 **7586 条**
（train 2602 / dev 2663 / eval 2321 冻结）。

**配额分母的口径（本轮澄清，容易读错）**：家族配额（confab≥40% / setting≥25% / ooc≥20%）
的分母是**缺陷样本**，正常样本**另**立模块、另算口径 —— §4.2.2 原文即"正常样本**另**按
'误报口径'造（每题材 ≥40）"，且三条下界之和只有 85%，本就给"另算"留了位置。
若把负例并进分母，confab 读作 **34.8%**，照本宣科的读者会以为"配额破了"去灌更多 confab ——
**正好毁掉同一节要求的误报率口径**。守卫 `test_normal_samples_do_not_dilute_the_family_quota`
把口径钉死（含反向断言：混成一个模块必须报警）。

**eval 层不加负例**：eval 是尺子且已冻结（决策 14），误报率口径在评测侧由手工语料
`eval-sets/` 的正常样本承担（§4.2.2 判据可判性表：每包 normal ≥40）。

**出厂体检（决策 36 的常规化）**：决策 36 是**偶然**发现的 ⇒ 这批新样本没有再靠运气。
探针固化为 `scripts/scenario_factory/leak_probe.py`（零 LLM 成本），三层实测：

| 探针 | train | dev | 结论 |
| --- | --- | --- | --- |
| 单特征可分性（材料 7 项 + 叙事 5 项；平衡准确率 ≥90% 即报警） | 最高 **56.0%** | 最高 **53.5%** | 无"题面写着答案" |
| 材料结构指纹（决策 36 的原始形态） | 缺陷 1093/1093 · 正常 221/221 **同形** | 1177/1177 · 210/210 同形 | 无"材料有洞" |
| 同材料跨标签 | **10 组** | **4 组** | 同一材料挂两种标签 ⇒ 材料**不可能**独自决定标签，模型必须读叙事 |
| 长度分布 | 中位 812 / 947（比 1.17，**正常反而更长**） | 830 / 901（1.09） | "短 ⇒ 通过"不成立 |
| 元叙述 | 3/1314 = 0.23% | 4/1387 = 0.29% | 与既有温和元词同量级 |

> 人读清单 §1 那句"实测不足目标一半 **221** 条"一度看着像"正常样本系统性偏短"——
> 实为**算术巧合**（缺陷侧 200 + 正常侧 21 = 221）。

**守卫 +9**（计数 512 → **521**）：`tests/test_leak_probe.py` 7 条 + 家族配额分母 1 条 +
清单双来源/过期 1 条；其中 **4 条是"必须报警"的回归夹具**（决策 36 的真实形态 / 长度捷径 /
标记捷径 / 清单过期）—— 探针不响就是摆设。**顺手抓到的两个自身错误**（都留了守卫）：
① 探针首版漏 `__main__` 守卫 ⇒ `python -m` 静默空转、退出码 0，看起来像"检查通过"；
② 我的测试夹具把数据根当层目录 ⇒ "健康数据无报警"是**假通过**（探头根本没读到数据）。
**这两个错误都是"必须报警"的夹具逼出来的** —— 只写"健康数据不报警"的测试会一路全绿地
放它们过去。另修一处真实缺陷：清单来源核对原先按**当前工作目录**解析相对路径
（换个目录就会去比别的文件、而"比错了却报告一致"最难发现）⇒ 改为**以仓库根为基准**。

**已知局限（本轮新记，均决定不改）**：

1. **材料 `当前地点` 恒为占位符「{题材}·场景」**（`cards.py:350`，实测 **3747/3747**，
   三层三类完全一致）。**不是类别泄漏**（均一 ⇒ 不含类别信息），但与生产（真地名）分布不同。
   修它要改 `cards.py` → 工厂版本变 → 三层重产 ≈¥96 / 45 小时 ⇒ **留给下一代数据集**。
2. **`_FACT_TPL` 五个模板没有时间维度**（物品/债务/身份/线索/承诺），而手工门禁语料里
   **15/448 = 3.3%** 是"时间/年数矛盾"（如"材料第 3 天 vs 叙事称修行十年"）。
   同一机制换一类事实，泛化风险不大 ⇒ **先照常训练，评测按切片看它到底掉不掉分**，
   用证据决定要不要扩模板（而不是现在猜着改）。

**成本**：① 合计 **¥5.07**（1550 次调用 / ¥0.0118 每条），账本累计 ¥118.78；
对照路径 B 的 ¥96。**代价差 19 倍，而产出完全等价。**

**人读清单同步**：judge 清单必须带上负例（**两个模块文件、一份清单**）——
漏掉它们等于把误报率那一侧排除在复核之外；清单头部逐个文件列 sha256 自证来源。
...
**第一段 · 程序判死（零成本）**：锚点出现在材料里 / 锚点不在叙事里 / 硬元叙述词（发现⑩）；
外加"锚点被疑问·否定·听说框架包着"的可疑标记。

**第二段 · 只问 LLM 语义**：中性提问（**不说**"这是编造的"）+ **反顺从装置** ——
答"材料有依据"就**必须逐字引用材料原文**，引不出即降级为"未判定"（给不出的"是"最廉价）。

**结果（1388 条全判，LLM 调用 1418 次，¥2.7）**：

| 层 | confab | 硬缺陷 | 可疑（人读） | 低风险 |
| --- | --- | --- | --- | --- |
| train | 457 | **0** | 55 | 402 |
| dev | 491 | **0** | 68 | 423 |
| eval | 440 | **0** | 49 | 391 |

- **硬缺陷 0** ⇒ 生成期那三道门（锚点不在材料 / 锚点在叙事 / 元叙述）确实在干活。
- **人读量 1388 → 172（−88%）**，其中 153 条来自程序句式、9 条来自 LLM"叙事没把 X 当断言"。
- **LLM 段不是摆设**：~30 条是程序段抓不到的（含 5 条"没断言"），值这 ¥2.7。

**两轮口径收紧（都由实测驱动，不是猜）**：

| 轮次 | 问题 | 实测 | 处置 |
| --- | --- | --- | --- |
| ① | "锚点 ±24 字内有 不/没/？/吗" | **90% 命中、全是假阳性**（`都没声响`／`连停顿都没有`／`用不着` 遍地都是） | 收紧成**紧贴锚点**的句式 → 13-16% |
| ② | `{锚点}吗/嘛` 算疑问 | 86 条命中里 **33 条是语气词**（「『银铃』嘛，我上回就瞧见了」= 断言） | 只认 `？`（回声疑问）→ 可疑 199 → 171 |

> 教训同发现⑥：把"必要条件筛查"当"产线过滤器"用，只会得到一堵噪声墙。
> 为此加了 **`--refresh`**：用日志里已有的 LLM 判定重算第一段（**零成本**）——
> 口径改一次就要重跑 1388 次（¥2.7/25 分钟）的话，没人会愿意改口径，而"口径不对"正是头号风险。

**提示词陷阱（实测修复）**：20 条"材料有依据"里 **19 条是同一个形态** —— 锚点「五十两」，
而材料状态栏写着「银两 50」（**玩家当前持有**）：模型把"他手里有 50 两"读成了"他欠 50 两"的依据。
补一句"数额相同不构成依据（要看有没有那笔**债务本身**）"后，19 条全部翻回"无依据"（¥0.04 定点重判，不重跑全量）。

**人读指定（pins）**：`data/route-a/<layer>/confab-triage-pins.txt`（**放层目录、不放 `.work/`**：
`.work/` 是机器草稿且被 gitignore，而 pins 是人的判断，必须进版本库 ——
放在 `.work/` 里它随一次 `git clean` 或换个克隆就没了，而它存在的全部理由就是"人的判断要留得住"）。
实测动因：**sc-32676** 的判定两次运行结论相反 —— 白芷角色卡写明「爱给人起外号」，
叙事是她**当场**给玩家起外号「寒鸦」（"跟喊隔壁二狗子似的"），不是在断言材料里没有的既成事实。
标签按宽读法可辩护、按严读法不成立 ⇒ **模型说不清的正是最该人读的**。
pins 在**读侧**叠加（人的判断压过机器，且不必为改一行注释重跑 LLM），报告里单列。
**判定翻转也留痕**（第二轮 28 条 = 语气词假阳性被清掉的那些）——"一次不稳定的判定不算干净的判定"。
> ⚠️ 一轮自伤：第二轮 `--refresh` 跑完又跑了一次（移 pins 之后），把第一遍记的 28 条翻转
> **覆写掉了**（报告显示"翻转 0 条"）。已把翻转痕迹改成**粘滞**（翻过就是翻过，
> `prev_level` 记最初那一档，守卫 `test_flip_mark_is_sticky_across_refresh`）；
> 那 28 条的去向以本档与提交信息为准（日志里的逐条痕迹已不可复原）。

**守卫 +18**（计数 521 → **539**）：`tests/test_confab_triage.py` 18 条 —— 程序三条判死 /
**两条"必须报警"的夹具**（决策 36 形态、清单过期）/ 语气词不是疑问（②轮的回归）/
反顺从装置（引不出出处就不算有依据）/ 解析失败 = 未判定 ≠ 通过 / 硬缺陷**不花钱问 LLM** /
续跑不重复付费 / `--redo` 覆盖并留翻转痕迹 / pins 压过"干净"。

**产出**：`reports/confab-triage-20260918.md`（六节：总览 / 硬缺陷 / 可疑 / 判定翻转 / 低风险抽查）
＋ `data/route-a/<layer>/.work/confab-triage-<layer>.jsonl`（机器可读、append-only、可续跑）。

**残余（交给训练评测看，不在本轮改数据）**：confab 的金额锚点**恒等于**状态栏的持有数
（`玩家欠X{amt}` 与 `玩家属性：银两 50` 同池，实测 19/1388 = 1.4%）。
这既是"数额巧合"的来源，也是一课难得的好样本（教模型"手里有钱 ≠ 有那笔债"）；
但**它会不会让 setting 切片（值被改写）掉分**，按计划"每家族分别报"的口径在评测里看。

**对抗式复核：试过，失败，已弃用**（详见 `reports/confab-adversarial-20260918.md`）。

想给 172 条可疑加一道"问法相反"的复核（不问"材料有没有依据"，而问"**这条标注错在哪**"）。
两轮实测：① 弹性判据 → train 55 条里 **50 条「站不住」（91%）**；② 收紧 B 并明列四条
"无效反驳"（角色说出／主角否认／数额巧合／语气平淡）→ 12 条试点仍 **7 条（58%）**。
**逐条读了 20 条，没有一条站得住**，套路是：把"角色在对话里说出"改叫"转述"（而对话
**正是**虚构事实发生的地方）、挑一句主角否认当作"叙述者推翻"（同段叙事先承认后否认）、
答非所问（锚点是「银铃」却引「欠沈砚五十两」）。

**根因**：判据的弹性来自**提问框架本身**，不是措辞 —— 与本次会话前两次踩的坑同形
（粗筛第一版 90% 假阳性、`吗/嘛` 当疑问）。**一个会响 60~90% 的筛子不是筛子。**
⇒ 代码/日志/自动报告全删，只留失败记录（仿 `label_audit.py` 先例：噪声太大的仪器删掉，
发现留下）。**数据没有因此发现新缺陷**。

顺带量出来的两个分布（都**不是**捷径，已全量核对）：① 叙事含"主角否认/想不起来"标记
confab 19.5% / ooc 17.1% / setting 11.4% / **normal 8.6%** —— 缺陷侧更高但二元规则平衡准确率
仅 **55%**，够不上线索；② "先承认后否认"的自我打架 confab 1.37%（19 条），读过 1 条，
文风含糊但**标签在两种读法下都成立** ⇒ 不改。
