...
| train | judge | 800 合格 | **803** | 257（24.2%） | ✅ | 供卡 1060（首估 1032 按 22.5% 丢弃；实测 24.3% → 补 28 张达标） |
| train | compress | 300 | **290** | 10（3.3%） | ✅ | 长输入 97/290 = **33%** ≥ 决策 18 门槛 20% |
| train | **合计** | — | **2091** | 269（11.4%） | ✅ | **train 层完成（2026-09-14）** |
...
- 残留观察（都不是缺陷，留给下一代数据集）：① **judge 类别偏斜** ooc 44% / confab 30% / setting 27%
  （卡面设计 37/28/34，差异全来自丢弃率不同：setting 存活 62.3%）；② judge 长度未兑现 144/803
  （同发现⑤ 一类，judge 无长度门槛）；③ compress 超提示词目标 156/290（留痕，非杀）。
