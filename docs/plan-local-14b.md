...
- 2026-09-12 · **judge 扩域基线（v2）：三包门禁全部通过** —— 江湖旧梦 / 问道长生 / 霓虹深处
  对抗类均 100%（ooc 12/12、setting 19~22、confab 16~20），误报率 **6% / 0% / 0%**
  （v1 为 22% / 17% / 22%）。**新账**：不可判定 9/213 = 4.2%（多为三轮皆空响应）——
  judge 的 2000 升级预算仍不足，建议提到 4000 再复测。→ `docs/plan-phase1-data.md` §3。
- 2026-09-12 · **阶段计划更新：新增 Step 1.5「世界包覆盖」**（`docs/plan-phase1-data.md` §3.5）——
  用户提出"训练/评测都围绕三个包是否合理"；结论：**不可以**（项目卖点 `import_story` 决定新包是常态）。
  拆解为三层（输入契约包无关 / 能力弱相关 / **评测尺子必须包无关**）+ 指出真问题是"**轴值太窄**"
  （现有三包题材不同但结构同质：4~6 节点、1~2 NPC、好感单向、单一货币）。
  方案：① 训练包集合 T（8~12 个新包）作数据主干，现有三包降为生产锚 ≤30%；
  ② 评测包与 T **按轴不相交**（新增守卫 `tests/test_pack_axis_coverage.py`，每轴 ≥1 取值只在评测集）；
  ③ **先做零样本跨包对照**（P1 校园·书信体 / P2 年代·双轨数值，≈¥0.3/半天）量化掉点，
  再决定造包规模（≤5pp 适度扩 / 10~20pp 以新包为主 / >20pp 先修提示词与输入形态）。
  使能器：`scripts/build_judge_corpus.py` 能从包自身资产生成语料 → 新包立门禁只需手写 6~12 条 ooc（~30 分钟）。
