...
| 项 | 确认内容 | 为什么关键 |
| --- | --- | --- |
| 上下文上限 | 原生/外推窗 ≥64K（推荐 128K 档），用真实 5 万 token 前缀实测不丢尾 | turn 输入中位 5.5 万 token |
...
2. 临时改 cli.py/web.py 常量，跑完还原——**注意工作区当前有未提交改动，先 `git stash` 隔离**；
