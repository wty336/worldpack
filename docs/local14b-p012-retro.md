...
| 阶段 4：B 轴 dedup | ⚠️ 原记「🎉 14B 10/10 = 100% **完胜** flash 40-50%」**已被 2026-09-11 重测推翻**：修复侧信道预算后 flash 同样 **10/10 = 100%** → **持平**（旧 40-50% 系 61.2% 调用顶满 50 预算所致，见 §6.2 勘误） |
...
### 6.2 B 轴 · dedup：原记「14B 完胜 flash」——**已被 2026-09-11 重测推翻（持平）**

> **勘误（2026-09-11，详见 `reports/sidechannel-budget-retest-20260911.md`）**：修复侧信道预算
> （dedup 50 → 500 + 空响应升级重试）后重跑，**flash 10/10 = 100%**。历史全量 usage 取证显示
> flash 的 456 次 dedup 调用中 **61.2% 顶满 50 预算**（14B 侧 0%）——原「40-50%」是预算 bug
> 的产物，不是能力值。因此本节「窄模块本地化第一实锤」的论据**不再成立**；处置结论（dedup
> 不训、留回归集）不变，但理由从"14B 完胜"改为"**两者无差距、不必训**"。

- flash 旧实测 40-50%（§5.3；现已知为 61.2% 调用顶满所致）；
- **14B 拦截 10/10 = 100%**，判定稳定；
- 新证据（2026-09-11 重测）：修复预算后 flash 亦 **10/10 = 100%**（单次调用 completion ≤ 228）→ 持平。
...
| 16 | 每次 `pytest` 全量回归都改写被 git 跟踪的产物（`saves/longrun-*.json`、`saves/smoke-*.json` 等） | 离线脚本把产物目录硬编码为 `saves/`（`SAVE_DIR = Path("saves")`），没有目录覆盖参数；测试用默认参数跑 → 写入仓库产物。内容确定性一致时 git status 看不出来，一旦引擎行为变化（如调用计数）就暴露成"莫名改动" | 脚本加 `--out-dir`（默认 `saves/`，保持生产行为）；**测试一律传 `tmp_path`**，并断言产物落在指定目录；离线回归后 `git status` 应只有预期改动 |
| 17 | 14B 在新包上 confab 拦 85~90%（**比已知三包还高**），差点据此下"无窄化 / 方向反转"结论 | 新包 confab 种子**撞说话人角色卡底线/禁忌**（如「留一栏，你署自己的名字」× 底线「不替他人的稿子署名」），判官多一条"设定矛盾"短路 → 拦截率虚高；两侧用例**难度不匹配**，比的是语料不是模型 | 跨包对照前先核对两侧"可拦截通路"构成：`scripts/card_hook_check.py --gate`（词面筛查，**必要不充分**——零共同二字的语义撞卡仍要人读）+ `tests/test_card_hook_check.py` 守 CI；改完语料**换尺子后两侧都重测**。实测：卡中性化后同一族用例 88% → 27.5%，与已知三包 32% 同难（`docs/plan-phase1-data.md` §3.5.6c） |
| 18 | 报告只记 `model=local-14b`，换量化/后端/`max_model_len` 后历史数字无法对账；且 `.env` 里 `deepseek-v4-flash` 是**别名**（服务端回显 `deepseek-flash`），`/v1/models` 里两个名字都没有 | 报告缺"部署指纹"（权重 repo / 窗口 / 服务实现）；供应商别名不体现在模型列表里 | 评测脚本统一写 `endpoint` 指纹（`game_agent/endpoint.py`：`base_url`/`model`/`model_root`/`max_model_len`/`served_by`），探测失败**只记 `probe_error` 不中断实验**；`not_listed` 与 `probe_error` 分开表达（"名字不在列表"≠"端点打不通"）；已接入 judge/extract/dedup/longrun/playthrough/baseline 六个报告 |
| 19 | "normal 误报率 0/18 = 0%"被读作"完美"（判据 ≤10% 通过） | **点估计伪装确定性**：18 个样本时 0 次的 95% Wilson 区间是 **0~17.6%**，横跨门限 → 该判据在此样本量下**不可判** | 所有比例类指标报 **Wilson 区间**并标注"是否可判"（`game_agent/evalmeta.py`）；把"每包每类配额"做成会 FAIL 的守卫（`scripts/eval_quota_check.py`），配额不足即"不可判 ≠ 达标"；报告加 `case_set_digest`（这次实际跑了哪些用例）+ `content_version`（语料版本），老数字作切片保留、新老不混报（`eval-sets/MANIFEST.md` §8） |
| 20 | 差点忽略"训练/评测数据会不会又聚焦到少数世界包" | 轴标签靠**人工声明**会自欺；模板级近重复（`ancient`/`xianxia` 的 identity 用例**同句换人名**，相似度 0.84）不会被肉眼发现 | 轴覆盖做成**声明 vs 机械事实**核对（era/stats/NPC 数/主线节点数从包 YAML 抽，改错了就 FAIL）+ holdout 轴上 **T∩E = ∅**（`scripts/axis_coverage_check.py`）；跨来源近似重复守卫（`scripts/near_dup_check.py`，二字组 Jaccard ≥0.6 即报）；首跑即抓出"结构轴未留出"+2 对模板近重复 |
| 21 | 真轨迹 normal 质检时**16/17 条被判官丢弃，且判词是空的** | **思考模式在真轨迹材料上停不下来**：材料 374~795 字（合成用例 420~570），预算 500 → reasoning 800 字；预算 2000 → reasoning 7152 字，`finish=length`、content 恒 0 → `parse_verdict("")` → unknown。更糟：**截断会改变判词**（4000 预算时草率输出"通过"，8000 时判"虚构"）——即"预算不足"不只是漏判，是**判错** | 侧信道关思考：`DEEPSEEK_DISABLE_THINKING=1` → `extra_body={"thinking":{"type":"disabled"}}`（`llm.complete_with_meta`；主回合 `run_turn` **不关**）。实测 reasoning=0、判词正常、17 次调用 0 重试、成本 ¥0.212 → **¥0.031（1/7）**。`thinking:disabled` 与 `reasoning_effort=minimal`/`chat_template_kwargs` 逐个试过，只有前者有效 |
| 22 | 真轨迹 normal 的**干净率只有 ~12%**（2/17） | flash 自己跑出来的轨迹里大量"重复发现同一事实""时间线冲突"——判官判违规（引原文）。所以"跑 40 回合就能凑 40 条 normal"不成立；按 12% 良率，40/包 × 5 包需 ~1700 回合（≈¥15~20 且慢） | 已实测并记录；**填 normal 的路线待定**（三选：① 接受 12% 良率多跑；② 用真轨迹**材料**（状态栏/facts/记忆真实）+ 另写干净叙事；③ 退回程序化 normal 并标注"非真轨迹"）。无论哪条，**材料真实性**（长材料 + 累积 facts/记忆）是这一轮最大的新资产 |
