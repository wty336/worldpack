# 本地 14B 实验 · 阶段 0-5 执行复盘（2026-09-08 ~ 09-11）

> 对应 `docs/plan-local-14b.md` Phase 0 的前段执行：阶段 0（L2 门禁 + 离线统计）、
> 阶段 1（vLLM 环境与接入冒烟）、阶段 2（实验支撑脚本）、阶段 3（flash 侧对照重跑）、
> 阶段 4（14B 侧门禁矩阵）、阶段 5（ICR 对拍 v1/v2）。
> 阶段 6 的正式结论将落 `reports/local14b-p0-<ts>.md`（文档 §10 滚动复盘区）。
...
| 阶段 3：flash 专项冒烟 | ✅ lore / reflect（0 洞察 0 矛盾，样本弱）/ injection / start / condition ×2 全过；⚠️ **dedup_test 拦截率 40-50% < 80%**（flash 基线退化，见 §4.3） |
...
### 5.3 ⚠️ dedup 基线退化（产品级红灯，建议独立立项）

- **现象**：dedup_test 拦截率 5/10（第一次）→ 4/10（重跑），两次**失败组不同**
  （第一次仅组10「剑法平平 vs 武功剑法很一般」放行；第二次组1/2/5/8/9/10 放行）。
- **已排除的变量**：PAIRS 固定 10 组（自 P1 未改）；dedup 调用 `temperature=0.0` 显式传参
  （memory.py:215）；memory.py dedup 逻辑 P1 后仅有 C 批次"反思门控"改动未触及去重。
- **结论**：P1 验收时（2026-09-07）拦截率 ≥80%，当前 40-50%——**v4-flash 服务端行为
  漂移**（2026-07-24 模型换代后持续演进）最可能；且 temp=0 下两次运行失败组不同，
  说明 DeepSeek 对 temp=0 的执行存在非确定性。
- **对实验的影响**：B 轴 dedup 对照线从"历史 80%"修正为"**flash 实测 40-50%**"；
  决策门判据（≥ flash−5pp）按实测值执行。
- **对产品的影响**：P1 门禁挂红——语义去重失效意味着重复事实会写入记忆桶（长局
  记忆质量隐患）。与 14B 实验无关，建议单独立项排查（语料侧：PAIRS 组10 的
  「剑法平平 vs 武功剑法很一般」其实语义距离较远，bigram 交集「玩家的」「剑法」
  触发 LLM 判定后放行——用例本身可能也偏难）。
...
1. **A 轴口径**：以 ICR v2（生产形态 --with-status + 污染剔除）为准；v1 数据仅作
   方法论对照记录（§7.1）。判据：一次通过率 ≥ flash−5pp、熔断率单独列报（14B v1 为
   12%，"熔断 0"硬线大概率不满足 → 预期走 Phase 1 路径）。
2. **成本**：flash 侧 cache 命中 84-96%，ICR 两轮总计 ≈¥2 内。
3. **B 轴对照线**：dedup 用 flash 实测 40-50%（§5.3）与 14B 100%（§6.2）对照；
   judge 用今日三包基线（100% 拦截 / 0% 误报）。
4. **长局与轨道事实**：长局零样本不可测（回合 0 熔断）；railed 已测出 0/3
   （事实提取层失败）——extract 蒸馏是 Phase 1 的重点模块，蒸馏后与协议稳定性一起复测。
5. **E 轴注意**：prefill 实测（~600 tok/s）低于 D.3 预估；ICR 样本 wall 时间已含
   TTFT+decode，v2 报告按前缀 token 分桶对照 flash 延迟。
6. **测量纪律沿用**：flash 命令不带任何 `DEEPSEEK_*` shell 前缀；14B 命令显式带
