...
> **2026-09-13 补记（修复 ①~⑥ 后重跑）**：三层出库 **261 条**（原 210），干净重跑段约 **¥2.71**（629 次调用）
> → 单位成本 **¥0.0104/样本** → 外推生产规模 2,100 条 **≈¥22**（比本报告 §1 的 ¥25 更低：④ 放开"超长"误杀后
> 丢弃率由 20% 降到 ~3%、同样调用数产出更多样本）。**日志累计** ¥5.2090（含被作废的老批 ¥2.5035、探针与
> 四轮轨道 2 复校、confab 门限诊断等），轨道 2 评委另计 **¥0.3318**（137 次调用）。
> 下表的**结构**与差异原因不变（长度缺陷仍是"假便宜"的主因，⑤ 只把标记改真实、未改产出长度）。
